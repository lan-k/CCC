...
##From https://github.com/agbarnett/covid_cccc
# 99_functions.R
...
blood_gases = function(
                       before_var='ecmo_worst_pa_o2_6hr_before',
                       after_var = 'pa_o2',
                       etype = NULL, 
                       logt=T){
  
  if (!is.na(etype)) {
    patients <- patients %>% filter(ecmo_type == etype)
  ...
  if (logt == T) { # log transform
    one = log(for_table$before) 
    ...
    one = for_table$before 
    two = for_table$after
    test = t.test(y=one, x=two, paired=TRUE, alternative='two.sided')
    # percent difference
    test$pdiff = 100*test$estimate
    test$conf.int = 100*test$conf.int
  }
